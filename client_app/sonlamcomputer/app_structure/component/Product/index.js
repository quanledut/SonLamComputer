...
import { StyleSheet, ScrollView, Text, View, Image, Dimensions, ImageBackground, FlatList, ActivityIndicator } from 'react-native';
...
class ProductViews extends Component {
    ...
    makeRemoteRequest() {
        console.log("in make remote")
        const { page, itemPerPage } = this.state;
        this.setState({
            loading: true
        });

        const data = this.fakeData(page,itemPerPage)
...
const styles = StyleSheet.create({
    ...
    productItem: {width: GET_SIZE(0.5, 1)-20, flex: 1, alignItems: 'center', marginTop: 5},
    productItemImage: {width: GET_SIZE(0.5, 1)-15, height: GET_SIZE(0.2)},
    productDescription: {backgroundColor: 'white', },
    productItemName: {width: GET_SIZE(0.5, 1)-15, fontSize: 25, padding: 10},
